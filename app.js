...
app.get("/buyers/search", (req, res) => {
  ...
  let sql = `
    SELECT 
      users.id AS user_id, 
      users.username, 
      writer_profiles.specialization, 
      writer_profiles.hourly_rate, 
      writer_profiles.available 
    FROM users 
    INNER JOIN writer_profiles ON users.id = writer_profiles.user_id 
    WHERE users.role = 'writer'`;
  const params = [];
  if (specialization) {
    sql += " AND writer_profiles.specialization = ?";
    params.push(specialization);
  }
  if (experience_level && experience_level !== "") {
    sql += " AND writer_profiles.experience_level = ?";
...
app.get("/profile/:id", (req, res) => {
  ...
  const selectWriter = `
    SELECT users.name, writer_profiles.bio, writer_profiles.experience_level, 
           writer_profiles.specialization, writer_profiles.hourly_rate, 
           writer_profiles.portfolio_url 
    FROM users 
    INNER JOIN writer_profiles ON users.id = writer_profiles.user_id 
    WHERE users.id = ?`;
  const selectProjects = `SELECT title FROM applications WHERE user_id = ?`;
  db.query(selectWriter, [writerId], (err, writerResults) => {
    if (err) {
      console.error(err);
      return res.status(500).send("Error fetching writer profile");
    }
    if (writerResults.length === 0) {
      return res.status(404).send("Writer not found");
    }
    const writer = writerResults[0];
    db.query(selectProjects, [writerId], (selectErr, projects) => {
      if (selectErr) {
        console.log(selectErr);
        return res.status(500).send("Error fetching projects");
      }
      writer.past_projects = projects;
      res.render("writer-profile.ejs", { writer: writer });
    });
  });
});

//descriptions of writer
app.get("/description", (req, res) => {
  ...
  db.query(selectProjects, (selectErr, projects) => {
    if (selectErr) {
      ...
      res.render("description.ejs", { projects: projects });
    }
  });
});
//checkout
...
// Add this route to your app.js or wherever your routes are defined
app.get("/portfolio", (req, res) => {
  const writerId = req.params.writerId; // Assuming you have writerId available
  const selectPortfolioItems = `SELECT * FROM portfolio_items WHERE writer_id = '${writerId}'`;
  db.query(selectPortfolioItems, (err, portfolioItems) => {
    if (err) {
      console.error(err);
      res.status(500).send("Error fetching portfolio items");
    } else {
      // Render portfolio.ejs with portfolioItems data
      res.render("portfolio.ejs", {
        writer: { name: "Writer Name", portfolio_items: portfolioItems },
      });
    }
  });
});
